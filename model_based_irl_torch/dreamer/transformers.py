...
class CausalSelfAttention(Module):
    ...
    def forward(self, x, key_padding_mask=None):
        ...
        ## add key_padding_mask to avoid attending to the padding tokens in the end of the sequence
...
class SelfAttentionBlock(Module):
    ...
    def forward(self, inputs):
        ...
        """
        x = inputs['batch']
        key_padding_mask = inputs['key_padding_mask'] if 'key_padding_mask' in inputs else None
        x = x + self.nets["attention"](self.nets["ln1"](x), key_padding_mask=key_padding_mask)
        x = x + self.nets["mlp"](self.nets["ln2"](x))
        outputs = {'batch': x, 'key_padding_mask': key_padding_mask}
        return outputs
...
class GPT_Backbone(Module):
    ...
    def forward(self, inputs):
        """
        inputs has two keys one is batch, the other is key_padding_mask
        """
        # inputs_batch = inputs['batch']
        # assert inputs_batch.shape[1:] == (self.context_length, self.embed_dim), inputs_batch.shape
        outputs = self.nets["transformer"](inputs)
        transformer_output = self.nets["output_ln"](outputs['batch'])
