...
from dreamer.prediction import ClassifierTrainer
...
def main(config):
    ...
    if config.debug:
        ...
        logger = tools.Logger(logdir, 0)#config.action_repeat * step)
    ...
    # observation_space, action_space, _, _, _  = tools.fill_expert_dataset_robocasa(config, success_eps, is_val_set=False, dataset_type='success')
    ...
    agent.eval()
    classifier_trainer = ClassifierTrainer(wm = agent._wm, logger = logger, device = config.device, config = config,
...
def make_classifier_dataset(episodes, config):
    sampler = tools.FullEpisodeSampler()
    if 'varied' in config.classifier_mode:
        generator = sampler.sample_episodes_with_varied_length(episodes, config.classifier_batch_length)
    else:
        generator = sampler.sample_full_episodes(episodes, config.classifier_batch_length, mode = config.classifier_padding_mode)
